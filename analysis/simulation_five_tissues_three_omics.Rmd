...
plot_gene_power_compare_group <- function(results_dir, runtag1, runtag2, thins, mapping_table) {
  ...
  for (thin in thins) {
    ...
    method2_results <- process_method(pipfs_method2, "Single")
    ...
    power_df <- rbind(
      calculate_stats(method1_results, "Multi-omics multi tissues"),
      calculate_stats(method2_results, "Expression from one tissue")
    )
    
    power_df$method <- factor(power_df$method, levels = c("Multi-omics multi tissue", "Expression from one tissue"))
...
## Multi-cTWAS reduces false positive rate
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.width=4, fig.height=4}
results_dir <- "/project/xinhe/shengqian/cTWAS_simulation/simulation_five_uncorrelated_tissues_three_omics/"
runtag1 <- "five_uncorrelated_tissues_three_omics"
...
runtag_list2 <- c(paste0(c("Whole_Blood","Liver","Heart_Left_Ventricle","Stomach","Lung"),"_expression"))
