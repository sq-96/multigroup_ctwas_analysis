...
```{r}
phenofs <- paste0(results_dir, runtag, "_simu",  paste(1, 1:10, sep = "-"), "-pheno.Rd")
pipfs <- paste0(results_dir, runtag, "_simu", paste(1, 1:10, sep = "-"), ".thin", 1, ".", "shared_all", ".", "ctwas",".minp08.mingene0.estimated_parameter.finemap_regions_res.RDS")
cau <- lapply(phenofs, function(x) {load(x);get_causal_id(phenores)})
df <- NULL
...
saveRDS(df,file = "/project/xinhe/shengqian/cTWAS_simulation/simulation_five_correlated_brain_tissues_three_omics/molecular_PIP_calibration_df.RDS")
```
...
for (i in 1:length(pipfs)) {
    ...
    true_genes <- unique(mapping_table[mapping_table$molecular_id %in% sapply(cau[[i]],function(x){unlist(strsplit(x, "[|]"))[1]}),"gene_name"])
    res$ifcausal <- ifelse(res$gene_name %in% true_genes, 1, 0)  
    res <- res[,c("gene_name","combined_pip","ifcausal","runtag")]
    colnames(res) <- c("gene","susie_pip","ifcausal","runtag")
    df <- rbind(df, res)
}
saveRDS(df,file = "/project/xinhe/shengqian/cTWAS_simulation/simulation_five_correlated_brain_tissues_three_omics/gene_PIP_calibration_df.RDS")
```
...
runtag_list2 <- c(paste0("expression_",c("Cerebellum","Cortex","Nucleus_accumbens","Caudate","Cerebellar_Hemisphere")),
                  paste0("splicing_",c("Cerebellum","Cortex","Nucleus_accumbens","Caudate","Cerebellar_Hemisphere")),
                  paste0("stability_",c("Cerebellum","Cortex","Nucleus_accumbens","Caudate","Cerebellar_Hemisphere")))
