...
for (trait  in traits){
  ...
  top_tissues <- get_top_tissues_phe(
    trait = trait, 
    n_top = 10,
    folder_results = folder_results
  )
  ...
  for (i in 1:length(tissue_combination)){
    ...
    file_param_pair <- paste0("/project/xinhe/xsun/multi_group_ctwas/23.multi_group_0515/pairwise_snakemake_outputs_bynumber/",trait,"/",trait,".",tissue_pair,".eqtlonly.thin1.shared_all.param.RDS")
  ...
  for (i in 1:length(tissue_combination)){
    ...
    file_param_pair <- paste0("/project/xinhe/xsun/multi_group_ctwas/23.multi_group_0515/pairwise_snakemake_outputs_bynumber/",trait,"/",trait,".",tissue_pair,".sqtlonly.thin1.shared_all.param.RDS")
    param_pair <- readRDS(file_param_pair)
    ctwas_parameters_pair <- summarize_param(param_pair, gwas_n, enrichment_test = "fisher")
