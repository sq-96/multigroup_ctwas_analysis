---
title: "Summary for multi-group analysis, e + s + st QTLs"
author: "XSun"
date: "2025-06-03"
output:
...
get_ctwas_file <- function(trait, tissue = NULL, folder_results,thin,var_struc, L) {
  # Build file paths
  if (is.null(tissue)) {
    file_ctwas_res_origin <- paste0(folder_results, "/", trait, "/", trait,".thin",thin,".",var_struc,".L",L, ".finemap_regions_res.RDS")
    file_ctwas_res_regionmerge <- paste0(folder_results, "/", trait, "/", trait,".thin",thin,".",var_struc,".L",L, ".regionmerge_finemap_regions_res.RDS")
    file_ctwas_res_ldmismatch <- paste0(folder_results, "/", trait, "/", trait,".thin",thin,".",var_struc,".L",L, ".ldmismatch_finemap_regions_res.RDS")
  } else {
    file_ctwas_res_origin <- paste0(folder_results, "/", trait, "/", trait, ".", tissue,".thin",thin,".",var_struc,".L",L, ".finemap_regions_res.RDS")
    file_ctwas_res_regionmerge <- paste0(folder_results, "/", trait, "/", trait, ".", tissue,".thin",thin,".",var_struc,".L",L, ".regionmerge_finemap_regions_res.RDS")
    file_ctwas_res_ldmismatch <- paste0(folder_results, "/", trait, "/", trait, ".", tissue,".thin",thin,".",var_struc,".L",L, ".ldmismatch_finemap_regions_res.RDS")
  }
...
for (trait in traits){
  ...
  file_ctwas_result <- get_ctwas_file(trait, tissue = "3qtls", folder_results = folder_results_multi, var_struc = "shared_all",L = 5,thin = 1)
  ctwas_res <- readRDS(file_ctwas_result)
  
  finemap_res <- ctwas_res$finemap_res
  finemap_res_gene <- finemap_res[finemap_res$type != "SNP",]
  
  p <- ggplot(finemap_res_gene, aes(x = abs(z), y = susie_pip)) +
