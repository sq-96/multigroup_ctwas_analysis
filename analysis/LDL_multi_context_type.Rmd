...
p <- ggplot(df, aes(x=reorder(group, -values), y=values, fill=group)) +
     ...
     xlab("")+
     theme_bw()+
     theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1), axis.text=element_text(size=20), axis.title.y = element_text(size=20),legend.position = "none")
...
intron_info <- data.frame()
for(i in unique(ctwas_gene_S_res$tissue)){
  intron_info_tmp <- get(load(paste0("/project/xinhe/shengqian/cTWAS_analysis/data/",i,"_Splicing_Mapping.RData")))
  intron_info <- rbind(intron_info,intron_info_tmp)
}
intron_info <- unique(intron_info)
