...
```{r, message=FALSE, warning=FALSE, echo=FALSE, fig.height=6, fig.width=15}
...
ggplot(df, aes(x = Method, y = Proportion, fill = Method)) +
  ...
  theme_minimal(base_size = 18) + grids(linetype = "dashed") + 
...
### Figure S12: Validation with IBD deepseek curated genes
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=6, fig.height=6}
...
data <- readRDS("/project/xinhe/shengqian/cTWAS_paper_figures/data/figure_data/avg_genes_per_locus.RDS")
out <- plot_avg_genes_per_locus_across_traits(data)
```

### Figure S12: Comparison of number of signals per region between M-cTWAS, Coloc and TWAS (LDL, histogram)
...
### Figure S13: Comparison of number of signals per region between M-cTWAS, Coloc and TWAS (other traits)
...
### Figure S14: Coloc examples at FASD1 locus
...
### Figure S15: Compare with TGFM: (HTN)
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.height=6, fig.width=8}
library(grex)
library(ggplot2)
...
combined_pip_by_group_multi_sig <- combined_pip_by_group_multi[1:100,]
combined_pip_by_context_multi_sig <- combined_pip_by_context_multi[1:100,]
combined_pip_by_context_multi_sig <- combined_pip_by_context_multi_sig[,c("gene_name","combined_pip","Artery_Aorta_pip","Liver_pip",
                                                                          "Artery_Tibial_pip","Whole_Blood_pip")]
...
### Figure S16: cTWAS helps identify causal modality and context (other traits)
...
data <- readRDS("/project/xinhe/shengqian/cTWAS_paper_figures/data/figure_data/novel_gene_category.RDS")
data <- data$data
p <- make_ctwas_category_boxplot(data)
print(p)
```

### Figure S17: Novel gene category for other traits
...
### Figure S18: Network analysis of candidate genes suggests novel pathways (IBD)
```{r,echo=FALSE, message=FALSE, warning=FALSE, fig.width= 5, fig.height= 5}
...
### Figure S19: Locus plot of highlighted gene
#### LDLR
<figure class="half">
...
```{r, eval=FALSE, echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.height=4, fig.width=3}
...
### Figure 6c: Do eQTLs explain epiQTLs? Independent PHE
```{r echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
### Figure 6d: Do eQTLs explain epiQTLs? few PIPs decreased after adding eQTLs
...
### Figure S20: Do eQTLs explain epiQTLs? Why eQTLs do not explain epiQTLs. Cell type specific mQTLs
...
all_cpg_ids <- fread("/project/xinhe/shengqian/cTWAS_epigenetic/data/all_cre_cpgs.bed")$V4
...
### Figure S21: Do eQTLs explain epiQTLs? GTEx is limited in power. Most mQTLs and caQTLs are eQTLs in MetaBrain eQTL
...
files <- c("/project2/xinhe/shared_data/multigroup_ctwas/GTEx_significant_associations/GTEx_Analysis_v8_eQTL/Brain_Cerebellar_Hemisphere.v8.signif_variant_gene_pairs.txt.gz",
           "/project2/xinhe/shared_data/multigroup_ctwas/GTEx_significant_associations/GTEx_Analysis_v8_eQTL/Brain_Cerebellum.v8.signif_variant_gene_pairs.txt.gz",
           "/project2/xinhe/shared_data/multigroup_ctwas/GTEx_significant_associations/GTEx_Analysis_v8_eQTL/Brain_Cortex.v8.signif_variant_gene_pairs.txt.gz",
           "/project2/xinhe/shared_data/multigroup_ctwas/GTEx_significant_associations/GTEx_Analysis_v8_eQTL/Brain_Nucleus_accumbens_basal_ganglia.v8.signif_variant_gene_pairs.txt.gz",
           "/project2/xinhe/shared_data/multigroup_ctwas/GTEx_significant_associations/GTEx_Analysis_v8_eQTL/Brain_Caudate_basal_ganglia.v8.signif_variant_gene_pairs.txt.gz")
...
```{r echo=FALSE,fig.height=4,fig.width=3}
driver <- dbDriver('SQLite')
db <- dbConnect(drv = driver, "/project2/xinhe/shared_data/multigroup_ctwas/weights/brain_mQTL.db")
query <- function(...) RSQLite::dbGetQuery(db, ...)
weights <- query("select * from weights")
RSQLite::dbDisconnect(db)
#MetaBrain_eQTL <- readRDS("/project/xinhe/shengqian/cTWAS_epigenetic/data/MetaBrain_eQTL/MetaBrain_eQTL_qvalue005.RDS")
cre_cpgs <- fread("/project2/xinhe/shengqian/cTWAS_epigenetics/data/all_cre_cpgs.bed")
weights <- weights[weights$gene %in% cre_cpgs$V4,]
...
cre_cpgs <- fread("/project/xinhe/shengqian/cTWAS_epigenetic/data/all_cre_cpgs.bed")
weights <- weights[weights$gene %in% cre_cpgs$V4,]
write.table(weights$rsid,file = "/project/xinhe/shengqian/cTWAS_epigenetic/data/all_cre_cpgs_topQTLs.txt",
...
cre_atac <- fread("/project2/xinhe/shengqian/cTWAS_epigenetics/data/all_cre_atac.bed")
weights <- weights[weights$gene %in% cre_atac$V4,]
write.table(weights$rsid,file = "/project/xinhe/shengqian/cTWAS_epigenetic/data/all_cre_atac_topQTLs.txt",
            sep = "\t",col.names = F,row.names = F,quote = F)
```


### Figure 6e: Do epiQTLs explain eqtls? Independent PHE
...
### Figure 6f: Do epiQTLs explain eqtlS? few PIPs decreased aftering adding epiQTLs
```{r echo=FALSE, message=FALSE, warning=FALSE, results='hide',fig.height=5, fig.width=5}
suppressMessages({
  suppressWarnings({
