...
plot_enrichment_uncorrelated(results_dir,runtag,paste(1, 1:10, sep = "-"),c(1),"shared_all","ctwas",".minp08.mingene0.parameters.RDS",enrichment_true)
```
...
snp_info <- fread("/project/xinhe/shengqian/cTWAS_epigenetic/data/UKBBLD_SNPs.txt")
gwas <- readRDS("/project2/xinhe/shared_data/multigroup_ctwas/gwas/ctwas_processed_zsnp/LDL-ukb-d-30780_irnt.preprocessed.z_snp.RDS")
gwas <- gwas %>% left_join(dplyr::select(as.data.frame(snp_info), id, pos, chrom), by = "id")
gwas$p <- z2p(gwas$z)
...
p1 <- plot_heatmap(heatmap_data = combined_pip_by_group_multi_sig, main = paste0("New genes identified by multigroup analysis, PIP>",PIP_cutoff),showPIP = T)
  
p2 <- plot_heatmap(heatmap_data = combined_pip_by_context_multi_sig, main = paste0("New genes identified by multicontext analysis, PIP>",PIP_cutoff),showPIP = T)
```
...
### Figure 6a: Enrichment of epiQTLs vs. eQTLs
```{r,echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.height=4, fig.width=10}
...
### Figure 6b: epiQTLs explain a larger fraction of h2g than eQTLs and different QTLs have limited overlap
```{r,echo=FALSE, message=FALSE, warning=FALSE, results='hide', fig.height=4, fig.width=10}
