...
get_results <- function(param_file,ctwas_file){
  ...
  prop_h2g <- as.data.frame(cbind(names(ctwas_parameters$prop_heritability),ctwas_parameters$prop_heritability,ctwas_parameters$group_pve))
  ...
  colnames(prop_h2g) <- c("group","%h2g","h2g")
  ...
  for(i in unique(finemap_res$group)){
    ...
    data <- matrix(c(snp_pip, snp_n, tissue_pip, tissue_n), nrow = 2, byrow = TRUE)
    data <- round(data)
    result <- fisher.test(data)
    pvalue_df <- rbind(pvalue_df,data.frame(group=i,pvalue=round(-log10(result$p.value),3)))
  }
