...
get_results <- function(gwas_id,gwas_name){
  ...
  df <- data.frame(
    value = c(coloc_data, multictwas_data,twas_data),
    group = c(rep("Coloc",length(coloc_data)),rep("Multi-cTWAS",length(multictwas_data)),rep("TWAS",length(twas_data)))
  )
...
get_results_mts <- function(gwas_id,gwas_name){
  ...
  twas_data <- as.vector(table(twas_res$region_id))
  # Combine data into a data frame
  df <- data.frame(
    value = c(coloc_data, multictwas_data,twas_data),
    group = c(rep("Coloc",length(coloc_data)),rep("Multi-cTWAS",length(multictwas_data)),rep("TWAS",length(twas_data)))
