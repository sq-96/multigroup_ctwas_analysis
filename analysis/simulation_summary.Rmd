...
```{r, fig.width= 6, fig.height= 3, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
results_dir <- "/project/xinhe/shengqian/cTWAS_simulation/simulation_four_tissues_expression/"
runtag = "four_tissues"
simutags <- paste(1, 1:5, sep = "-")
phenofs <- paste0(results_dir, runtag, "_simu", simutags, "-pheno.Rd")
...
```{r, fig.width= 3, fig.height= 3, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
plot_df$method <- factor(plot_df$method, levels=c("TWAS", "coloc", "cTWAS", "multi-cTWAS"))
...
colset = c("#ebebeb", "#87CEFA", # FOCUS
           ...
           "#ebebeb", "#b2df8a", # MR-JTI
           "#ebebeb", "goldenrod", #PMR-Egger
           "#ebebeb", "#87CEFA", # SMR
           "#ebebeb", "#fb8072", # cTWAS
           "#ebebeb", "#bebada") # coloc

f1 <- ggbarplot(plot_df, 
          x = "method", 
          y = "count", 
          add = "mean_se", 
          fill = "ifcausal", 
...
```{r, fig.width= 4, fig.height= 5, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
results_dir <- "/project/xinhe/shengqian/cTWAS_simulation/simulation_four_tissues_expression/"
runtag = "four_tissues"
simutags <- paste(1, 1:5, sep = "-")
...
for(i in 1:5){
  ...
  coloc_genes <- c(get_top_genes_coloc(colocfs1[i],"expression_Liver"),get_top_genes_coloc(colocfs2[i],"splicing_Liver"),
                   get_top_genes_coloc(colocfs3[i],"expressionAdipose_Subcutaneous"),get_top_genes_coloc(colocfs4[i],"splicing_Adipose_Subcutaneous"),
                   get_top_genes_coloc(colocfs5[i],"expression_Lung"),get_top_genes_coloc(colocfs6[i],"splicing_Lung"))
...
```{r, fig.width= 4, fig.height= 4, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
...
f2 <- ggbarplot(plot_df, 
          ...
          ylab="Count", 
          xlab="",
          palette = colset) + grids(linetype = "dashed") + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) + scale_y_break(c(60, 150),space=0.5,scales="free")
```
...
### PIP partition across tissues
```{r, fig.width= 6, fig.height= 5, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
results_dir <- "/project/xinhe/shengqian/cTWAS_simulation/simulation_seven_correlated_tissues_expression/"
runtag = "7corr"
simutags <- paste(1, 1:5, sep = "-")
...
### Compare to other methods
```{r, fig.width= 8, fig.height= 4, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
results_dir <- "/project/xinhe/shengqian/cTWAS_simulation/simulation_five_groups/"
runtag = "five_groups"
simutags <- paste(1, 1:5, sep = "-")
phenofs <- paste0(results_dir, runtag, "_simu", simutags, "-pheno.Rd")
susieIfs <- paste0(results_dir, runtag, "_simu",simutags, "_UKBBLD", ".estimatedL.finemap_regions_res.RDS")
susieIfs1 <- paste0(results_dir, runtag, "_simu",simutags, "_UKBBLD", ".estimatedL.averaged.finemap_regions_res.RDS")
zgenefs <- paste0(results_dir, runtag, "_simu",simutags, "_UKBBLD", ".z_gene.RDS")
plot_df <- data.frame()
for(i in 1:5){
  cau <- lapply(phenofs, function(x) {load(x);get_causal_id(phenores)})
  ...
  different_enrichment <- get_top_genes(susieIfs[i])
  
  plot_df <- rbind(plot_df,
                   data.frame(simutag=i,
                              method="Multiple groups",
                              count=length(intersect(different_enrichment,causal_genes)),
                              ifcausal=T))
  plot_df <- rbind(plot_df,
                   data.frame(simutag=i,
                              method="Multiple groups",
                              count=length(different_enrichment)-length(intersect(different_enrichment,causal_genes)),
                              ifcausal=F))
  
  averaged_enrichment <- get_top_genes(susieIfs1[i])
  
  plot_df <- rbind(plot_df,
                   data.frame(simutag=i,
                              method="One group",
                              count=length(intersect(averaged_enrichment,causal_genes)),
                              ifcausal=T))
  plot_df <- rbind(plot_df,
                   data.frame(simutag=i,
                              method="One group",
                              count=length(averaged_enrichment)-length(intersect(averaged_enrichment,causal_genes)),
                              ifcausal=F))
...
plot_df$method <- factor(plot_df$method, levels=c("One group","Multiple groups"))
...
ggbarplot(plot_df, 
          ...
          xlab="",
          width = 0.6,
          palette = colset) + grids(linetype = "dashed")+
  theme(
    axis.text = element_text(size = 23),   # Font size for axis text
    axis.title = element_text(size = 23), # Font size for axis labels
    legend.text = element_text(size = 23),# Font size for legend text
    legend.title = element_text(size = 23)# Font size for legend title
  )
```

## Simulation 6: enrichment pvalue
```{r echo=FALSE}
results_dir <- "/project/xinhe/shengqian/cTWAS_simulation/simulation_one_tissue/one_tissue_simu1-1"
runtag = "one_tissue"
simutags <- paste(1, 1:100, sep = "-")
for(i in simutags){
  ctwas_parameters <- readRDS(paste0(results_dir, runtag, "_simu",i, "_UKBBLD", ".ctwas_parameters.RDS"))
  print(ctwas_parameters$group_pvalue)
}
```