...
pdf(file = "./prior_7unccor.pdf", width = 8, height = 5)
...
plot_par_7(truth,est,xlabels = c("Liver","Lung","Blood","Adipose","Artery","Heart","Stomach"),ylim=c(0,0.025),ylab="Prior inclusion",cex.lab=1.3)
dev.off()
```

### Estimated PVE
```{r, fig.width= 8, fig.height= 5, message=FALSE, warning=FALSE, echo=FALSE}
pdf(file = "./pve_7unccor.pdf", width = 8, height = 5)
...
est <- rbind(cbind(1,y1),cbind(2,y2),cbind(3,y3),cbind(4,y4),cbind(5,y5),cbind(6,y6),cbind(7,y7))
plot_par_7(truth,est,xlabels = c("Liver","Lung","Blood","Adipose","Artery","Heart","Stomach"),ylim=c(0,0.06),ylab="PVE",cex.lab=1.3)
dev.off()
```
...
pdf(file = "./PIPs_7unccor.pdf", width = 8, height = 4)
...
#pdf(file = "./prop_7unccor.pdf", width = 8, height = 5)
prop_df <- get_pip_distr(results_dir,runtag,configtag,simutags,thin,sample_size,PIP_threshold)
prop_df <- prop_df[,2:8]/prop_df$total_pips
prop_df <- prop_df %>% summarise(across(everything(), list(mean = mean)))
long_df <- pivot_longer(prop_df, cols = everything(), names_to = "Tissue", values_to = "Mean")
tissues <- c("Liver","Lung","Blood","Adipose","Artery","Heart","Stomach")
means <- as.numeric(prop_df[1, ])
plot.new()
grid()
par(new = TRUE)
barplot(means, 
        names.arg = tissues, 
        xlab = "", 
        ylab = "Proportion of PIP", 
        col = c("#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00", "#f781bf", "#a65628"),
        border = NA,
        ylim = c(0,0.5),
        cex.lab=1.3)
#dev.off()
