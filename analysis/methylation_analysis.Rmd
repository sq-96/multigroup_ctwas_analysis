...
wholeblood_E_res <- anno_finemap_res(wholeblood_E_res$finemap_res,
                                ...
                                add_position = TRUE,
                                use_gene_pos = "mid")
wholeblood_E_res <- subset(wholeblood_E_res, type != "SNP" & susie_pip > 0.75 & !is.na(cs))
...
colon_E_res <- anno_finemap_res(colon_E_res$finemap_res,
                                ...
                                add_position = TRUE,
                                use_gene_pos = "mid")
colon_E_res <- subset(colon_E_res, type != "SNP" & susie_pip > 0.75 & !is.na(cs))
...
```{r}
set1 <- wholeblood_E_res$gene_name
set2 <- colon_E_res$gene_name
set3 <- combined_pip_by_type[combined_pip_by_type$combined_pip>0.8,]$gene_name
# Prepare a palette of 3 colors with R colorbrewer:
library(RColorBrewer)
...
aa <- venn.diagram(
  ...
  category.names = c("wholeblood E" , "Colon E", "All"),
  filename = NULL,
  lwd = 2,
  lty = 'blank',
  fill = myCol
...
```{r}
snp_map <- readRDS("/project/xinhe/shengqian/cTWAS_analysis/data/ibd_multi/ibd_multi.snp_map.RDS")
ibd_multi_res <- readRDS("/project/xinhe/shengqian/cTWAS_analysis/data/ibd_multi/ibd_multi.finemap_regions_res.RDS")
ibd_multi_res <- anno_finemap_res(ibd_multi_res$finemap_res,
                                snp_map = snp_map,
                                mapping_table = gene_annot,
                                add_gene_annot = TRUE,
                                map_by = "molecular_id",
...
```{r, echo=FALSE, fig.width=15, fig.height=13}
param <- readRDS("/project/xinhe/shengqian/cTWAS_analysis/data/ibd_multi_old/ibd_multi.param.RDS")
gwas_n <- 59957
colors <- c(  "#1f77b4", "#ff7f0e", "#2ca02c", "#d62728",  "#9467bd", "#8c564b", "#e377c2", "#7f7f7f",  "#bcbd22",  "#17becf",  "#f7b6d2",  "#c5b0d5",  "#9edae5", "#ffbb78",  "#98df8a",  "#ff9896" )
make_convergence_plots(param, gwas_n,colors = colors)
```
...
### Percent of heritability with methylation lasso models
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
ctwas_parameters <- readRDS("/project/xinhe/shengqian/cTWAS_analysis/data/ibd_multi_old/ibd_multi.ctwas_parameters.RDS")
pve_pie_by_type_multi <- plot_piechart(ctwas_parameters = ctwas_parameters, colors = colors, by = "type")
pve_pie_by_context_multi <- plot_piechart(ctwas_parameters = ctwas_parameters, colors = colors, by = "context")
gridExtra::grid.arrange(pve_pie_by_type_multi,pve_pie_by_context_multi, ncol = 2)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
gwas_n <- 59957
finemap_res <- readRDS("/project/xinhe/shengqian/cTWAS_analysis/data/ibd_multi_old/ibd_multi.finemap_regions_res.RDS")
gene_annot <- readRDS("/project2/xinhe/shared_data/multigroup_ctwas/weights/mapping_files/PredictDB_E_S_M_mapping.RDS")
susie_alpha_res <- anno_susie_alpha_res(finemap_res$susie_alpha_res,
                                        mapping_table = gene_annot,
                                        map_by = "molecular_id",
...
### cTWAS parameters with methylation lasso models (50kb, h2 pvalue<0.00001)
```{r, echo=FALSE, fig.width=15, fig.height=13}
param <- readRDS("/project/xinhe/shengqian/cTWAS_analysis/data/wbc_multi/wbc_multi.param.RDS")
gwas_n <- 563946
colors <- c(  "#1f77b4", "#ff7f0e", "#2ca02c", "#d62728",  "#9467bd", "#8c564b", "#e377c2", "#7f7f7f",  "#bcbd22",  "#17becf",  "#f7b6d2",  "#c5b0d5",  "#9edae5", "#ffbb78",  "#98df8a",  "#ff9896" )
make_convergence_plots(param, gwas_n,colors = colors)
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
ctwas_parameters <- readRDS("/project/xinhe/shengqian/cTWAS_analysis/data/wbc_multi/wbc_multi.ctwas_parameters.RDS")
pve_pie_by_type_multi <- plot_piechart(ctwas_parameters = ctwas_parameters, colors = colors, by = "type")
pve_pie_by_context_multi <- plot_piechart(ctwas_parameters = ctwas_parameters, colors = colors, by = "context")
gridExtra::grid.arrange(pve_pie_by_type_multi,pve_pie_by_context_multi, ncol = 2)
```
...
### Top cTWAS genes with methylation single QTL models
```{r, echo=FALSE, message=FALSE, warning=FALSE}
finemap_res <- readRDS("/project/xinhe/shengqian/cTWAS_analysis/data/wbc_multi/wbc_multi.finemap_regions_res.RDS")
gene_annot <- readRDS("/project2/xinhe/shared_data/multigroup_ctwas/weights/mapping_files/PredictDB_E_S_M_mapping.RDS")
