...
```{r, fig.width=16,fig.height=8,echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
```{r, fig.width=16,fig.height=4,echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
library(EnsDb.Hsapiens.v86)
for(i in result$region_id){
  ens_db <- EnsDb.Hsapiens.v86
  p <- make_locusplot(finemap_res_multi,
                 region_id = i,
                 #locus_range = c(115237972,115337972),
                 ens_db = ens_db,
                 weights = weights,
                 highlight_pip = 0.8,
                 filter_protein_coding_genes = FALSE,
