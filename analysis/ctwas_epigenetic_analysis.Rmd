...
mapping_table <- readRDS("/project2/xinhe/shared_data/multigroup_ctwas/weights/mapping_files/gene_mapping.RDS")
predictdb_mapping_table <- readRDS("/project2/xinhe/shared_data/multigroup_ctwas/weights/mapping_files/PredictDB_mapping.RDS")
mapping_table <- rbind(mapping_table,predictdb_mapping_table)
...
finemap_res_multi <- anno_finemap_res(finemap_res,
                                      ...
                                      add_position = TRUE,
                                      use_gene_pos = "mid")
finemap_res_multi <- finemap_res_multi[finemap_res_multi$type=="caQTL",]
finemap_res_multi <- finemap_res_multi[!is.na(finemap_res_multi$cs),]  
#all_cre_cpgs <- unlist(strsplit(finemap_res_multi$molecular_id, "_"))
#write.table(all_cre_cpgs,file = "/project/xinhe/shengqian/cTWAS_epigenetic/data/all_cre_cpgs.txt",sep = "\t",quote = F,row.names = F,col.names = F)
```
...
```{r}
###merged CpGs and intersect with ATAC-peaks
all_cre_atac <- finemap_res_multi$molecular_id
all_cre_atac <- mapping_table[mapping_table$molecular_id %in% all_cre_atac,]

all_cre_atac$start <- as.numeric(all_cre_atac$start)-1
all_cre_atac$end <- as.numeric(all_cre_atac$end) + 1
all_cre_atac$chr <- paste0("chr",as.numeric(all_cre_atac$chrom))
all_cre_atac <- all_cre_atac[,c("chr","start","end","molecular_id")]
write.table(all_cre_atac,file = "/project2/xinhe/shengqian/cTWAS_epigenetics/data/all_cre_atac.bed",sep = "\t",quote = F,row.names = F,col.names = F)
```



```{r}
###merged CpGs and intersect with ATAC-peaks (random CpGs)
set.seed(2025)
all_cpgs <- readRDS("/project2/xinhe/shared_data/multigroup_ctwas/weights/mapping_files/gene_mapping.RDS")
all_cpgs <- all_cpgs[!grepl("Peak", all_cpgs$molecular_id), ]
random_cpgs <- sample(all_cpgs$molecular_id, 10000, replace = FALSE)
all_cpgs <- all_cpgs[all_cpgs$molecular_id %in% random_cpgs,]
all_cpgs$start <- as.numeric(all_cpgs$start)-1
all_cpgs$end <- as.numeric(all_cpgs$end) + 1
all_cpgs$chr <- paste0("chr",as.numeric(all_cpgs$chrom))
all_cpgs <- all_cpgs[,c("chr","start","end","molecular_id")]
write.table(all_cpgs,file = "/project2/xinhe/shengqian/cTWAS_epigenetics/data/random_cpgs.bed",sep = "\t",quote = F,row.names = F,col.names = F)
```
