...
gwas_detail <- merge(gwas_selected_gtex, gtex, by.x = "ID", by.y = "Tag", all.x = TRUE)
gwas_detail <- gwas_detail[match(gwas_selected_gtex$traits,gwas_detail$traits),]
gwas_detail <- gwas_detail[c(setdiff(names(gwas_detail), "ID"), "ID")]

gwas_detail <- gwas_detail[,c("trait_category","sub_category","traits","abbreviation.x","Consortium","PUBMED_Paper_Link","Phenotype","Sample_Size","Population","ID")]

DT::datatable(gwas_detail,caption = htmltools::tags$caption( style = 'caption-side: left; text-align: left; color:black;  font-size:150% ;',"Munro's GWAS"),options = list(pageLength = 10) )
```
...
DT::datatable(gwas_detail,caption = htmltools::tags$caption( style = 'caption-side: left; text-align: left; color:black;  font-size:150% ;','GWAS used in TGFM paper'),options = list(pageLength = 10) )
```
