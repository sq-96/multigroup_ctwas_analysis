...
plot_PIP <- function(results_dir, runtag,  simutags, ...){
   phenofs <- paste0(results_dir, runtag, "_simu", simutags, "-pheno.Rd")
   susieIfs <- paste0(results_dir, runtag, "_simu",simutags, "_UKBBLD.finemap_regions.res.RDS")
   f1 <- caliPIP_plot_csindex(phenofs, susieIfs, ...) 
   return(f1)
}
plot_PIP_merged <- function(results_dir, runtag,  simutags, ...){
   phenofs <- paste0(results_dir, runtag, "_simu", simutags, "-pheno.Rd")
   susieIfs <- paste0(results_dir, runtag, "_simu",simutags, "_UKBBLD.finemap_merged_regions.res.RDS")
   f1 <- caliPIP_plot_csindex(phenofs, susieIfs, ...) 
   return(f1)
}
```
...
`````{r, fig.width= 3, fig.height= 5, message=FALSE, warning=FALSE, echo=FALSE}
...
plot_par_2(truth,est,xlabels = c("GTEX LD", "UKBB LD"),ylim=c(0,0.025),ylab="Prior inclusion")
```
...
est <- rbind(cbind(1,y1),cbind(2,y2))
plot_par_2(truth,est,xlabels = c("GTEX LD", "UKBB LD"),ylim=c(0,0.1),ylab="PVE")
```

### PIP Calibration Plot based on UKBB LD (filter out cs index 0)
```{r, fig.width= 8, fig.height= 4, echo=FALSE, warning=FALSE}
f1 <- plot_PIP(results_dir, runtag, simutags, main = "no merged regions")
f2 <-  plot_PIP_merged(results_dir, runtag, simutags, main = "merged regions")
