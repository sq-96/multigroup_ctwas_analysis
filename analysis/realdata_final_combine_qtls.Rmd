...
### Comparing eQTL, e+s QTL, e+s+st QTL and 4 qtls
...
p4 <- plot_piechart_topn(ctwas_parameters = ctwas_parameters,colors = colors,by = "type",title = NULL)
...
pie4 <- fix_panel_size(p4)

# Calculate widths of each gtable (plot + legend)
widths <- unit.c(grobWidth(pie1), grobWidth(pie2),grobWidth(pie3), grobWidth(pie4))

# Arrange plots with their natural widths
p <- grid.arrange(pie1, pie2, pie3, pie4,
                  ncol = 4, 
                  widths = widths,
                  top = paste0(trait)
)
