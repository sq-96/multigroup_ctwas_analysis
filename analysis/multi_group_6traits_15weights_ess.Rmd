...
compute_pip_per_cs <- function(combined_data, susie_data, pip_threshold) {
  # dplyr::filter and process the susie_data in a vectorized way
  final_details <- susie_data %>%
    dplyr::filter(in_cs & complete.cases(cs)) %>%  # Only rows in credible sets and without NA in cs
    group_by(gene_name, susie_set) %>%      # Group by gene and credible set
    summarise(
      total_PIP_CS = round(sum(susie_alpha, na.rm = TRUE), digits = 3), # Sum of susie_alpha
      num_molecular_traits_CS = n(),                                   # Count of rows
      ids_pip_CS = paste0(id, "(", round(susie_alpha, digits = 3), ")", collapse = ", "), # Concatenated IDs
      .groups = "drop" # Drop grouping structure after summarization
    ) %>%
    dplyr::filter(total_PIP_CS > pip_threshold) %>% # dplyr::filter by pip_threshold
    group_by(gene_name) %>%                 # Group by gene_name
    dplyr::filter(n() >= 2) %>%                    # Keep genes with 2 or more rows
...
```{r, echo=T, message=FALSE, warning=FALSE, fig.width=3, fig.height=3}
...
pip_per_cs <- compute_pip_per_cs(combined_pip_by_group_sig_multi, susie_alpha_res_multi,0.3)
...
make_locusplot(finemap_res_multi,
               region_id = "2_141761033_143761915",
               ens_db = ens_db,
               weights = weights_multi,
               highlight_pip = 0.8,
               filter_protein_coding_genes = TRUE,
